...
// static assertion to check that the structure is indeed packed
static_assert(12 == sizeof(AMCOM_ObjectState), "12 != sizeof(AMCOM_ObjectState)");
...
typedef struct AMPACKED {
    ...
    float angle;
    /// action to perform (0 = none, 1 = drop spark)
    uint8_t action;
} AMCOM_MoveResponsePayload;
// static assertion to check that the structure is indeed packed
static_assert(5 == sizeof(AMCOM_MoveResponsePayload), "4 != sizeof(AMCOM_MoveResponsePayload)");
