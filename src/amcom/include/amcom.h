 ...
 * PAYLOAD - the actual data of the packet. Can be from 0 to 200 bytes long.
 */
 ...
 */
typedef void (*AMCOM_PacketHandler)(AMCOM_Packet const *packet, void *userContext);
...
// Compile-time assertion to check that the header structure is indeed packed
static_assert(5 == sizeof(AMCOM_PacketHeader), "5 != sizeof(AMCOM_PacketHeader)");

// Compile-time assertion to check that the packet structure is indeed packed
